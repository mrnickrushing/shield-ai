...
DEFAULT_LESSONS = [
        ...
        "quiz_questions": [
            ...
            },
            {
                ...
                "options": [
                    ...
                    "Click it quickly so malware cannot load",
    ...
    },
    {
        ...
        "content": """## Why gift cards?\n\nGift cards are scammers' favourite payment method because they are nearly irreversible once redeemed, anonymous, and the victim does the work.\n\n## Common setups\n\n- **Fake tech support** — Pay Microsoft with iTunes cards.\n- **IRS impersonation** — Threatens arrest unless you pay back taxes with Google Play cards.\n- **Grandparent scam** — Someone pretending to be a grandchild needing bail money.\n- **Prize scam** — You won a prize but must pay a processing fee in gift cards first.\n\n## The rule\n\nNo government agency, utility company, bank, or legitimate business will ever ask you to pay with gift cards. Ever.\n\nIf someone insists on gift card payment, hang up.""",
        "threat_category": "gift_card_scam",
        "difficulty": "beginner",
        "estimated_minutes": 3,
        "quiz_questions": [
            {
                "question": "Someone calls claiming to be the IRS and says you owe back taxes payable by Google Play cards. What should you do?",
                "options": [
                    "Buy the cards — the IRS has real authority",
                    "Ask them to call back later",
                    "Hang up — the IRS never requests gift card payment",
                    "Pay half now to negotiate",
                ],
                "answer_index": 2,
            },
        ],
    },
    {
        ...
        "content": """## The 30-second URL check\n\n### 1. Look at the real domain\nThe domain is between https:// and the first single /. In https://secure.paypal.com.attacker.net/... the real domain is attacker.net, not paypal.com.\n\n### 2. Check for typosquatting\nCommon substitutions: paypa1.com, rn instead of m, 0 instead of o.\n\n### 3. Check the TLD\nLegitimate companies rarely use .xyz, .top, .click, or .zip for login pages.\n\n### 4. URL shorteners\nbit.ly and tinyurl.com hide the real destination. Use a URL expander before clicking.\n\n### 5. HTTP vs HTTPS\nHTTPS does not guarantee safety, but HTTP on a login page is always wrong.""",
        "threat_category": "url_scam",
        "difficulty": "beginner",
        "estimated_minutes": 4,
        "quiz_questions": [
            {
                "question": "What is the real domain in: https://paypal.com.secure-login.net/verify",
                "options": [
                    "paypal.com",
                    "secure-login.net",
                    "paypal.com.secure-login.net",
                    "verify",
                ],
                "answer_index": 1,
            },
    ...
    },
    {
        ...
        "content": """## What is a romance scam?\n\nA romance scam is when a criminal builds a fake romantic relationship online to eventually ask for money.\n\n## Warning signs\n\n1. **They look too good** — Stock-photo-perfect profile. Do a reverse image search.\n2. **They escalate fast** — Professing love within days or weeks.\n3. **They can never meet** — Always an excuse: military deployment, overseas contract, family emergency.\n4. **They ask for money** — Eventually there is always a crisis requiring urgent funds.\n5. **Unusual payment methods** — Wire transfers, crypto, gift cards.\n\n## How to verify\n\n- Reverse image search their profile photos (Google Images, TinEye).\n- Video call them unscripted.\n- Ask specific, verifiable questions about their claimed location.""",
        "threat_category": "romance_scam",
        "difficulty": "beginner",
        "estimated_minutes": 5,
        "quiz_questions": [
            {
                "question": "Someone you met online says they are in the military overseas and asks you to wire money for a medical emergency. What should you do?",
                "options": [
                    "Wire the money — military people are trustworthy",
                    "Send a smaller amount first to test",
                    "Stop contact and report to the FTC — this is a classic romance scam script",
                    "Ask them to FaceTime first, then decide",
                ],
                "answer_index": 2,
            },
        ],
    },
    {
        ...
        "content": """## The three pillars\n\n### 1. Strong, unique passwords\nUse a password manager (1Password, Bitwarden, Apple Keychain) to generate and store passwords. Never reuse passwords across sites.\n\n### 2. Two-factor authentication (2FA)\nEnable 2FA on every account that supports it. Authenticator apps are more secure than SMS. SMS 2FA can be bypassed by SIM swap attacks.\n\n### 3. Secure recovery options\nKeep your recovery email and phone number up to date. Store backup codes in a safe place.\n\n## Quick wins today\n\n1. Change any reused password on your email account.\n2. Enable 2FA on your email and bank.\n3. Check haveibeenpwned.com to see if your email appeared in a data breach.""",
        "threat_category": "account_security",
        "difficulty": "beginner",
        "estimated_minutes": 4,
        "quiz_questions": [
            {
                "question": "Which 2FA method is most resistant to SIM swap attacks?",
                "options": [
...
def get_lessons_for_user(db: Session, user_id: str, threat_category: str | None = None) -> list[dict]:
    ...
    progress_map = {
        p.lesson_id: p
        for p in db.query(EducationProgress).filter(EducationProgress.user_id == user_id).all()
    }
    ...
    for lesson in lessons:
        ...
        result.append({
            ...
            "threat_category": lesson.threat_category,
