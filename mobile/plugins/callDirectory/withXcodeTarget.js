...
function withCallDirectoryXcodeTarget(config) {
  return withXcodeProject(config, async (config) => {
    ...
    await writeCallDirectoryExtensionFiles(platformProjectRoot, appIdentifier);
    ...
    // Already added (e.g. re-running prebuild without --clean) — nothing
    // further to do. The `xcode` library stores target-name comments with
    // literal surrounding quotes, so the lookup must include them too.
...
module.exports = { withCallDirectoryXcodeTarget };
