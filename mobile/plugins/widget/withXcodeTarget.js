...
function withWidgetXcodeTarget(config) {
  return withXcodeProject(config, async (config) => {
    ...
    await writeWidgetExtensionFiles(platformProjectRoot, appIdentifier);
    ...
    const swiftFilePath = getSwiftFilePath(platformProjectRoot);
    ...
    pbxProject.addSourceFile(swiftFilePath, { target: target.uuid }, pbxGroupKey);
    ...
    for (const key in configurations) {
      ...
      if (
        ...
        buildSettingsObj["IPHONEOS_DEPLOYMENT_TARGET"] = "17.0";
      }
    }
...
module.exports = { withWidgetXcodeTarget };
