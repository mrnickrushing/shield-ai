...
function withSafariXcodeTarget(config) {
  return withXcodeProject(config, async (config) => {
    ...
    await writeSafariExtensionFiles(platformProjectRoot, appIdentifier);
    ...
    // Already added (re-running prebuild without --clean).
    ...
    // The web-extension payload (manifest + scripts) ships in Resources.
    for (const resourcePath of getResourceFilePaths(platformProjectRoot)) {
      pbxProject.addResourceFile(resourcePath, { target: target.uuid }, pbxGroupKey);
    }
...
module.exports = { withSafariXcodeTarget };
