    ...
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl }}>
      <FadeIn>
        ...
        >
          ...
          <Eyebrow style={{ marginBottom: spacing.sm }}>COMMUNITY INTEL</Eyebrow>
          <Text style={{ color: colors.text, fontSize: 24, fontWeight: "900", letterSpacing: -0.6, marginBottom: 6 }}>
            See the playbook before it hits you.
          </Text>
          <Text style={{ color: colors.textMuted, fontSize: 14, lineHeight: 21 }}>
            Active scam patterns identified across the Shield AI network.
          </Text>
        </Surface>
      </FadeIn>
      ...
      <FadeIn delay={100}>
        <Eyebrow style={{ marginBottom: spacing.sm }}>Active Threat Patterns</Eyebrow>
        ...
        {isLoading ? (
          ...
          <Surface style={{ alignItems: "center", paddingVertical: spacing.xl }}>
            ...
            <Text style={{ color: colors.textMuted, fontSize: 13, textAlign: "center", marginTop: 4 }}>
              Be the first to report a scam and help the community.
            </Text>
          </Surface>
        ) : (
          (patterns ?? []).map((p: any) => (
            <Surface key={p.id} style={{ marginBottom: spacing.sm }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                <Text style={{ color: colors.text, fontWeight: "700", fontSize: 15, flex: 1, letterSpacing: -0.2 }}>{p.name}</Text>
                <View
                  style={{
                    backgroundColor: withAlpha(boostColor(p.risk_score_boost), "22"),
                    borderRadius: 4,
                    paddingHorizontal: 8,
                    paddingVertical: 3,
                ...
                >
                  <Text style={{ color: boostColor(p.risk_score_boost), fontSize: 11, fontWeight: "800" }}>
                    +{p.risk_score_boost} risk
                  </Text>
                </View>
              </View>
              {!!p.description && (
      ...
      {!reportsError && (myReports ?? []).length > 0 && (
        <FadeIn delay={140}>
          ...
          {(myReports ?? []).map((r: any) => (
            <Surface key={r.id} style={{ marginBottom: spacing.sm, flexDirection: "row", alignItems: "center", gap: spacing.md }}>
              <View style={{ flex: 1 }}>
              ...
              <View
                style={{
                  backgroundColor: r.status === "approved" ? withAlpha(colors.safe, "22") : colors.surface,
                  borderRadius: radius.sm,
                  paddingHorizontal: 8,
                  paddingVertical: 3,
                  borderWidth: 1,
                  borderColor: r.status === "approved" ? withAlpha(colors.safe, "44") : colors.border,
                }}
              >
                <Text style={{ color: r.status === "approved" ? colors.safe : colors.textMuted, fontSize: 11, fontWeight: "700", textTransform: "uppercase" }}>
                  {r.status}
                </Text>
              </View>
            </Surface>
