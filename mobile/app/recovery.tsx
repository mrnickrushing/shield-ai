...
export default function RecoveryScreen() {
  ...
  const createIncident = useMutation({
    mutationFn: (type: string) =>
      ShieldAPI.createIncident({ incident_type: type, linked_scan_id: params.scanId }),
    onSuccess: (data: any) => {
  ...
  const updateIncident = useMutation({
    mutationFn: (completedSteps: string[]) =>
      ShieldAPI.updateIncident(incidentId!, { steps_completed: completedSteps }),
  });

  const addEvidence = useMutation({
    mutationFn: () =>
      ShieldAPI.addEvidence(incidentId!, {
    ...
    return (
      <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg }}>
        ...
        <Text style={{ color: colors.textMuted, fontSize: 14, marginBottom: spacing.lg }}>
          Select the type of scam to get a guided recovery plan.
        </Text>
        {SCAM_TYPES.map((t) => (
          <Pressable
            key={t.key}
  ...
  if (phase === "steps") {
    return (
      <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg }}>
        ...
        <Text style={{ color: colors.textMuted, fontSize: 14, marginBottom: spacing.lg }}>
          Work through each step. Tap to mark complete.
        </Text>
            ...
            >
              <View style={{ flexDirection: "row", alignItems: "flex-start", gap: spacing.sm, marginBottom: 6 }}>
                <View style={{
                  ...
                  backgroundColor: done ? colors.safe : "transparent",
                  borderWidth: 2, borderColor: done ? colors.safe : colors.border,
                  alignItems: "center", justifyContent: "center", marginTop: 2,
                }}>
                  {done && <Text style={{ color: "#000", fontSize: 11, fontWeight: "900" }}>✓</Text>}
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: done ? colors.safe : colors.text, fontWeight: "700", fontSize: 16, marginBottom: 4 }}>
                    {step.title}
                  </Text>
                  <View style={{ flexDirection: "row", marginBottom: 6 }}>
                    <View style={{
                      backgroundColor: (URGENCY_COLOR[step.urgency] ?? colors.textMuted) + "22",
                      borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2,
                    }}>
                      <Text style={{ color: URGENCY_COLOR[step.urgency] ?? colors.textMuted, fontSize: 10, fontWeight: "700", textTransform: "uppercase" }}>
                        {step.urgency}
                      </Text>
                    </View>
                  </View>
                  ...
                  {step.contacts?.map((c: any) => (
                    <Text key={c.label} style={{ color: colors.primaryBright, fontSize: 13, marginTop: 6 }}>
                      → {c.label}: {c.value}
                    </Text>
                  ))}
                </View>
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg }}>
      ...
      <Text style={{ color: colors.textMuted, fontSize: 14, marginBottom: spacing.lg }}>
        Add notes, URLs, transaction IDs, or any other details for your report.
      </Text>
      ...
      <TextInput
        ...
        style={{
          ...
          padding: spacing.md, marginBottom: spacing.sm, fontSize: 15,
        }}
      />
      <TextInput
        ...
        style={{
          ...
          fontSize: 15, textAlignVertical: "top", minHeight: 120,
        }}
      />
      ...
      <Pressable
        ...
        style={{
          backgroundColor: evidenceText ? colors.primary : colors.surface,
          borderRadius: radius.md, padding: spacing.lg,
          alignItems: "center", marginBottom: spacing.md,
        }}
      >
        <Text style={{ color: evidenceText ? "#fff" : colors.textMuted, fontWeight: "700", fontSize: 16 }}>
          Save Evidence
        </Text>
      </Pressable>
