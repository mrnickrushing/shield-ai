...
export default function EducationScreen() {
  ...
  const { data: lessons, isLoading } = useQuery({
    queryKey: ["lessons", threat_category],
    queryFn: () => ShieldAPI.listLessons(threat_category),
  });
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg }}>
      ...
      <Text style={{ color: colors.textMuted, fontSize: 14, marginBottom: spacing.lg }}>
        Short lessons to help you spot scams before they happen.
      </Text>
        ...
        (lessons ?? []).map((lesson: any) => (
          ...
          >
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
              <Text style={{ color: lesson.completed ? colors.safe : colors.text, fontWeight: "700", fontSize: 16, flex: 1 }}>
                {lesson.completed ? "✓ " : ""}{lesson.title}
              </Text>
              {lesson.quiz_score != null && (
                <Text style={{ color: colors.primaryBright, fontSize: 13, fontWeight: "700" }}>{lesson.quiz_score}%</Text>
              )}
            </View>
            <Text style={{ color: colors.textMuted, fontSize: 13, marginBottom: spacing.sm }}>{lesson.summary}</Text>
            <View style={{ flexDirection: "row", gap: spacing.sm }}>
              <View style={{
                backgroundColor: (DIFF_COLOR[lesson.difficulty] ?? colors.textMuted) + "22",
                borderRadius: 4, paddingHorizontal: 8, paddingVertical: 3,
              }}>
                <Text style={{ color: DIFF_COLOR[lesson.difficulty] ?? colors.textMuted, fontSize: 11, fontWeight: "700" }}>
