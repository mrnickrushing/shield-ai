...
export default function LessonScreen() {
  ...
  const { data: lesson, isLoading } = useQuery({
    queryKey: ["lesson", id],
    queryFn: () => ShieldAPI.getLesson(id),
  });
        ...
        <View style={{ marginTop: spacing.xl }}>
          ...
          {questions.map((q: any, qi: number) => (
            <View key={qi} style={{ marginBottom: spacing.lg }}>
              <Text style={{ color: colors.text, fontSize: 15, fontWeight: "600", marginBottom: spacing.sm, lineHeight: 22 }}>
                {qi + 1}. {q.question}
              </Text>
              {q.options.map((opt: string, oi: number) => {
                ...
                return (
                  <Pressable
                    ...
                    style={{
                      backgroundColor: correct ? "#0f2a18" : wrong ? "#2a0f0f" : selected ? colors.surfaceActive : colors.surface,
          ...
          {!submitted ? (
            <Pressable
              ...
              style={{
                backgroundColor: answers.length >= questions.length ? colors.primary : colors.surface,
