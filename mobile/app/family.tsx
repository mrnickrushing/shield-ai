...
export default function FamilyScreen() {
  ...
  const addContact = useMutation({
    ...
    onSuccess: () => { setShowAdd(false); setName(""); setPhone(""); setEmail(""); qc.invalidateQueries({ queryKey: ["trusted-contacts"] }); },
  });
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.bg }} contentContainerStyle={{ padding: spacing.lg }}>
      ...
      <Text style={{ color: colors.textMuted, fontSize: 14, marginBottom: spacing.lg }}>Trusted contacts are people you can escalate alerts to if you spot something suspicious.</Text>
      ...
      {(contacts as any[]).length === 0 && !showAdd && (
        <View style={{ backgroundColor: colors.surface, borderRadius: radius.lg, padding: spacing.xl, alignItems: "center", marginBottom: spacing.lg }}>
          ...
          <Text style={{ color: colors.textMuted, fontSize: 13, textAlign: "center" }}>Add a trusted friend or family member to share scam alerts with.</Text>
        </View>
      )}

      {(contacts as any[]).map((c: any) => (
        <View key={c.id} style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: radius.lg, padding: spacing.lg, marginBottom: spacing.md, flexDirection: "row", alignItems: "center" }}>
          <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: colors.primary + "33", alignItems: "center", justifyContent: "center", marginRight: spacing.md }}>
      ...
      {showAdd ? (
        <View style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: radius.lg, padding: spacing.lg, marginBottom: spacing.md }}>
          ...
          {([{label:"Name *",value:name,setter:setName,placeholder:"Full name"},{label:"Phone",value:phone,setter:setPhone,placeholder:"+1 555 000 0000"},{label:"Email",value:email,setter:setEmail,placeholder:"their@email.com"}] as const).map((f) => (
            <View key={f.label} style={{ marginBottom: spacing.sm }}>
              <Text style={{ color: colors.textMuted, fontSize: 12, marginBottom: 4 }}>{f.label}</Text>
              <TextInput value={f.value} onChangeText={f.setter as any} placeholder={f.placeholder} placeholderTextColor={colors.textMuted}
                style={{ backgroundColor: colors.bg, borderColor: colors.border, borderWidth: 1, borderRadius: radius.md, color: colors.text, padding: spacing.md, fontSize: 15 }} />
            </View>
          ))}
            ...
            <View style={{ flexDirection: "row", gap: spacing.sm }}>
              {REL_LABELS.map((r) => (
                <Pressable key={r} onPress={() => setRel(r)}
                  style={{ backgroundColor: rel === r ? colors.primary : colors.bg, borderColor: rel === r ? colors.primary : colors.border, borderWidth: 1, borderRadius: 20, paddingHorizontal: spacing.md, paddingVertical: spacing.sm }}>
          ...
          <View style={{ flexDirection: "row", gap: spacing.sm }}>
            <Pressable onPress={() => setShowAdd(false)} style={{ flex: 1, backgroundColor: colors.bg, borderRadius: radius.md, padding: spacing.md, alignItems: "center", borderWidth: 1, borderColor: colors.border }}>
              <Text style={{ color: colors.textMuted, fontWeight: "600" }}>Cancel</Text>
            </Pressable>
            <Pressable onPress={() => { if (name) addContact.mutate(); }} style={{ flex: 2, backgroundColor: name ? colors.primary : colors.surface, borderRadius: radius.md, padding: spacing.md, alignItems: "center" }}>
              <Text style={{ color: "#fff", fontWeight: "700" }}>Add Contact</Text>
            </Pressable>
          </View>
        </View>
      ) : (
        <Pressable onPress={() => setShowAdd(true)} style={{ backgroundColor: colors.primary, borderRadius: radius.md, padding: spacing.lg, alignItems: "center" }}>
