"""
Explicitly set PROVISIONING_PROFILE_SPECIFIER for the SafariWebExtension
target in the Xcode project file after xcode-project use-profiles has run.

usage: python3 patch_safari_extension_profile.py <profile_name>

Same rationale as patch_share_extension_profile.py: the extension's bundle ID
is stored as a variable reference ($(PRODUCT_BUNDLE_IDENTIFIER).safari-extension)
...
safariext_bundle = f"{bundle_id}.safari-extension"


def find_safariext_profile_name():
    """Return the Name of the installed safariext-extension provisioning profile."""
    for f in sorted(glob.glob(f"{profiles_dir}/*.mobileprovision")):
        ...
        if "safari-extension" in app_id.lower():
            name = p.get("Name", "")
...
def patch_pbxproj(pbxproj_path, profile_name):
    ...
    XCBuildConfiguration block that belongs to the SafariWebExtension
    target (identified by BUNDLE_IDENTIFIER or PRODUCT_BUNDLE_IDENTIFIER
    matching the safariext bundle ID, or by the literal variable
    reference).
    """
    with open(pbxproj_path) as f:
        lines = f.readlines()

    safariext_id_markers = [
        safariext_bundle.lower(),
        "safariwebextension",
        "safari-extension",
    ]
    ...
    while i < len(lines):
        ...
        if in_block:
            ...
            if any(m in lower for m in safariext_id_markers):
                is_safariext_block = True

            if brace_depth <= -2 and stripped.endswith("};"):
                if is_safariext_block:
    ...
    if not blocks_to_patch:
        print(f"[patch] no SafariWebExtension build configurations found in {pbxproj_path}")
    ...
    print(f"[patch] found {len(blocks_to_patch)} SafariWebExtension block(s) to patch")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        profile_name = find_safariext_profile_name()
        if not profile_name:
            print("[patch] ERROR: could not find safariext extension profile")
            sys.exit(1)
