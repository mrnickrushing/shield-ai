"""
Explicitly set PROVISIONING_PROFILE_SPECIFIER for the WidgetExtension
...
is stored as a variable reference ($(PRODUCT_BUNDLE_IDENTIFIER).widget)
...
def find_widget_profile_name():
    """Return the Name of the installed widget-extension provisioning profile."""
...
def patch_pbxproj(pbxproj_path, profile_name):
    ...
    XCBuildConfiguration block that belongs to the WidgetExtension
    target (identified by BUNDLE_IDENTIFIER or PRODUCT_BUNDLE_IDENTIFIER
    matching the widget bundle ID, or by the literal variable
    reference).
    """
    ...
    while i < len(lines):
        ...
        if in_block:
            ...
            if any(m in lower for m in widget_id_markers):
    ...
    if not blocks_to_patch:
        print(f"[patch] no WidgetExtension build configurations found in {pbxproj_path}")
    ...
    print(f"[patch] found {len(blocks_to_patch)} WidgetExtension block(s) to patch")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        profile_name = find_widget_profile_name()
        if not profile_name:
            print("[patch] ERROR: could not find widget extension profile")
            sys.exit(1)
