"""
Idempotent setup for the CallDirectoryExtension provisioning profile.

Steps:
  1. Ensure App ID com.shieldai.app.calldirectory exists with App Groups capability.
  2. Find the team's App Store distribution certificate.
  3. Delete any stale distribution profile for the calldirectory App ID.
  4. Create a fresh profile (including App Groups) and save it to
     scripts/call_directory_extension.mobileprovision for commit to the repo.
    ...
    res = api("POST", "/v1/bundleIds", {
        "data": {
            ...
            "attributes": {
                ...
                "name": "Shield AI Call Directory Extension",
                "platform": "IOS",
            },
        }
    })
...
def main():
    ...
    print(f"\n=== call-directory-extension signing setup: {calldirectory_bundle} ===", flush=True)

    bid_id = find_or_create_bundle_id(calldirectory_bundle)
    ensure_app_groups(bid_id)
    delete_stale_profiles(bid_id)
    cert_id = find_distribution_cert()
    dest_path = os.path.join(REPO_ROOT, "scripts", "call_directory_extension.mobileprovision")
    uuid, app_groups = create_and_install_profile(
        bid_id, cert_id, "Shield AI Call Directory Extension AppStore", dest_path
    )
    ...
    print(f"\n✓ Call directory extension profile ready (UUID={uuid})", flush=True)
