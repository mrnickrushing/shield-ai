"""
Idempotent setup for the MessageFilterExtension provisioning profile.

Steps:
  1. Ensure App ID com.shieldai.app.messagefilter exists with App Groups capability.
  2. Find the team's App Store distribution certificate.
  3. Delete any stale distribution profile for the messagefilter App ID.
  4. Create a fresh profile (including App Groups) and save it to
     scripts/message_filter_extension.mobileprovision for commit to the repo.
    ...
    res = api("POST", "/v1/bundleIds", {
        "data": {
            ...
            "attributes": {
                ...
                "name": "Shield AI Message Filter Extension",
                "platform": "IOS",
            },
        }
    })
...
def ensure_associated_domains_on_main_app(bundle_id):
    """The MAIN app needs the Associated Domains capability for its
    `messagefilter:<api-domain>` entitlement — iOS refuses to defer SMS
    queries to the network without it."""
    res = api("GET", f"/v1/bundleIds?filter[identifier]={bundle_id}")
    if not res.get("data"):
        raise RuntimeError(f"Main app bundle ID {bundle_id} not found in App Store Connect")
    main_bid = next(
        (b for b in res["data"] if b["attributes"]["identifier"] == bundle_id),
        res["data"][0],
    )["id"]

    caps = api("GET", f"/v1/bundleIds/{main_bid}/bundleIdCapabilities")
    if any(
        c["attributes"]["capabilityType"] == "ASSOCIATED_DOMAINS"
        for c in caps.get("data", [])
    ):
        print("  main app Associated Domains: already enabled", flush=True)
        return

    print("  main app Associated Domains: enabling...", flush=True)
    api("POST", "/v1/bundleIdCapabilities", {
        "data": {
            "type": "bundleIdCapabilities",
            "attributes": {"capabilityType": "ASSOCIATED_DOMAINS", "settings": []},
            "relationships": {
                "bundleId": {"data": {"type": "bundleIds", "id": main_bid}}
            },
        }
    })
    print("  main app Associated Domains: enabled", flush=True)
    print(
        "  NOTE: the main app's provisioning profile must be regenerated to pick "
        "this up — CI's fetch-signing-files does that automatically.",
        flush=True,
    )
...
def main():
    ...
    print(f"\n=== message-filter-extension signing setup: {messagefilter_bundle} ===", flush=True)

    ensure_associated_domains_on_main_app(bundle_id)
    bid_id = find_or_create_bundle_id(messagefilter_bundle)
    ensure_app_groups(bid_id)
    delete_stale_profiles(bid_id)
    cert_id = find_distribution_cert()
    dest_path = os.path.join(REPO_ROOT, "scripts", "message_filter_extension.mobileprovision")
    uuid, app_groups = create_and_install_profile(
        bid_id, cert_id, "Shield AI Message Filter Extension AppStore", dest_path
    )
    ...
    print(f"\n✓ Message filter extension profile ready (UUID={uuid})", flush=True)
